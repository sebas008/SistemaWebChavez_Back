...
public class AtencionService : IAtencionService
{
    ...
    public async Task<List<AtencionDto>> ListAsync(int? idObra, int? idAlmacenOrigen, int? idAlmacenDestino, string? estado, CancellationToken ct)
        => (await _repo.ListAsync(idObra, idAlmacenOrigen, idAlmacenDestino, estado, ct)).Select(a => new AtencionDto
        {
            ...
            Fecha = a.Fecha,
            IdObra = a.IdObra,
            IdAlmacenOrigen = a.IdAlmacenOrigen,
            IdAlmacenDestino = a.IdAlmacenDestino,
            MetodoAtencion = a.MetodoAtencion,
            Estado = a.Estado,
    ...
    {
        var header = await _repo.GetByIdAsync(idAtencion, ct);
        if (header == null) return null;

        var detalle = await _repo.Detalle_ListByAtencionAsync(idAtencion, ct);

        return new AtencionDto
        {
            IdAtencion = header.IdAtencion,
            Codigo = header.Codigo,
            Fecha = header.Fecha,
            IdObra = header.IdObra,
            IdAlmacenOrigen = header.IdAlmacenOrigen,
            IdAlmacenDestino = header.IdAlmacenDestino,
            MetodoAtencion = header.MetodoAtencion,
            Estado = header.Estado,
            Observacion = header.Observacion,
            Detalle = detalle.Select(d => new AtencionDetalleDto
            {
                IdAtencionDetalle = d.IdAtencionDetalle,
                IdItem = d.IdItem,
                CantidadPlanificada = d.CantidadPlanificada,
                CantidadAtendida = d.CantidadAtendida,
                Comentario = d.Comentario
            }).ToList()
        };
    }

    public Task<List<AtencionBandejaDto>> ListarBandejaAsync(CancellationToken ct)
    ...
    public async Task<AtencionRegistrarResponseDto> RegistrarDesdeAlmacenInternoAsync(AtencionRegistrarRequestDto req, CancellationToken ct)
    {
        ...
        var detalle = req.Detalle.Select(d => new AtencionDetalle
        {
            IdItem = d.IdItem,
            CantidadPlanificada = d.CantidadPlanificada,
            CantidadAtendida = d.CantidadAtendida,
        ...
        var result = await _repo.RegistrarDesdeAlmacenInternoAsync(
            req.IdObra,
            req.IdAlmacenOrigen,
            req.IdAlmacenDestino,
        ...
        return new AtencionRegistrarResponseDto
        {
